extends Node
# Autoload GifPool. Fetches GIF spritesheets from the local curation server, caches
# them under user://, slices frames into ImageTexture arrays, and hands out
# per-session animations to Background and Obstacle nodes. If the server is
# down the pool stays empty and consumers fall back to static textures.
...
var obstacle_gifs: Array = []
var background_gifs: Array = []

var _session_bg: GifAnim = null
var _bg_idx: int = 0
var _pending: Array = []
...
func _on_pool_fetched(result: int, code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
...
func _download_next() -> void:
	...
	var abs_dir: String = OS.get_user_data_dir() + "/gif_cache/" + gif_id
	DirAccess.make_dir_recursive_absolute(abs_dir)

	var req := HTTPRequest.new()
	add_child(req)
	req.request_completed.connect(func(result: int, code: int, _hdrs: PackedStringArray, body: PackedByteArray) -> void:
		req.queue_free()
...
func _load_spritesheet(entry: Dictionary, cache_path: String) -> void:
	...
	var raw_polys: Variant = entry.get("polygons", [])
...
func _load_subject_spritesheet(entry: Dictionary, cache_path: String) -> void:
	var anim: Variant = entry.get("_anim")
	if anim == null:
		return
	var f := FileAccess.open(cache_path, FileAccess.READ)
	if not f:
	...
	for i in frame_count:
		var region := Rect2i(i * frame_w, 0, frame_w, frame_h)
		var fimg := img.get_region(region)
		(anim as GifAnim).subject_frames.append(ImageTexture.create_from_image(fimg))
...
func get_obstacle_gif() -> GifAnim:
	if obstacle_gifs.is_empty():
		return null
	return obstacle_gifs[randi() % obstacle_gifs.size()]

func get_background_gif() -> GifAnim:
	return _session_bg

func cycle_background_gif() -> GifAnim:
	if background_gifs.is_empty():
		return null
